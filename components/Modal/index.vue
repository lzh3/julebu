<template>
  ...
  >
    <view
      :class="['modal-wrap',status]"
      v-if="status"
    >
      <image
        :src="`../../static/image/${status}.png`"
        :class="status"
      />
    ...
    <!-- // 登录页面 -->
    <view
      class="login-wrap modal-wrap success"
      v-if="login_status === 'login'"
    >
      <text
        class="title login text-green"
        :class="isSuccess?'login_suc':'login_err'"
      >{{msg}}</text>
      <button
        type="default"
        v-show='showlogin'
    ...
    <!-- 注册页面 -->
    <view
      class="modal-wrap error"
      v-if="reg_status === 'reg_error'"
    >
      <text
        class="title"
        :class="{reg_err:reg_status==='reg_error'}"
      >{{msg}}</text>
      <button
        type="primary"
        @click="loginFn"
