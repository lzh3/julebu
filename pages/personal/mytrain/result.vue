    ...
    <view class="content">
      <view class="item" v-for="(v,index) of list" :key="index">
        <view class="test-option-name">
          ...
          <image
            class="pd"
            src="../../../static/image/icon/personal/right.png"
            v-if="v.userAnswerRights"
          ></image>
          <image
            class="pd"
        ...
        <view class="test-options" v-if="!v.multiple">
          ...
          >
            <label
              class="uni-list-cell"
              v-for="(u, m) in v.answer"
              :key="u.title"
            >
              <view class="radio">
                <radio
                  :value="u.code"
                  :checked="u.code == v.userAnswer"
                  disabled
                />
              </view>
        ...
        <view v-else class="test-options">
          <checkbox-group @change="checkboxChange" class="checkbox-group">
            <label
              ...
              :key="u.title"
            >
              <!-- v.userAnswer.includes(u.code) && 'checkout-disabled' -->
              <checkbox
        ...
        <!-- 错误提示 -->
        <view class="error-tip" v-if="!v.userAnswerRights">
          正确答案：<text>{{displayRight(v.right)}}</text>
          <!-- 正确答案：<text>{{displayRight(v.right)}} -->
        </view>
      </view>
    </view>
  </view>
  ...
  data() {
    return {
      ...
      examinationIndexUrl:
        this.type === "h5"
          ? "/examination/index"
          : "https://amd.mcooks.cn/api/examination/index",
    };
  },
  onLoad(opts) {
